...
class AntiRecoilApp:
    def __init__(self, root):
        self.root = root
        self.root.title("🎯 ANTI-RECOIL APP")
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = tk.Label(main_frame, text="🎯 ANTI-RECOIL APP", 
                              font=('Arial', 20, 'bold'), 
                              fg='#00ff00', bg='#000000')
        ...
        # Start button
        self.start_button = tk.Button(main_frame, text="START ANTI-RECOIL", 
                                     font=('Arial', 14, 'bold'),
                                     bg='#00ff00', fg='#000000',
        ...
        tk.Label(control_frame, text="🎯 ANTI-RECOIL CONTROL", 
                font=('Arial', 14, 'bold'), 
        ...
        tk.Label(info_frame, text="📊 ANTI-RECOIL INFO", 
                font=('Arial', 14, 'bold'), 
                fg='#00ff00', bg='#111111').pack(pady=8)
        
        self.info_label = tk.Label(info_frame, 
                                  text="Anti-recoil app ready - auto-updating active", 
                                  font=('Arial', 11), 
        ...
        tk.Label(debug_frame, text="📊 ANTI-RECOIL DEBUG", 
                font=('Arial', 14, 'bold'), 
                fg='#00ff00', bg='#111111').pack(pady=8)
        ...
        tk.Label(hotkey_frame, text="⌨️ ANTI-RECOIL HOTKEYS", 
                font=('Arial', 14, 'bold'), 
    ...
    def toggle_script(self):
        ...
        if not self.running:
            self.running = True
            self.status_label.config(text="🟢 ANTI-RECOIL RUNNING", fg='#00ff00')
            self.start_button.config(text="STOP ANTI-RECOIL", bg='#ff0000')
            self.debug_label.config(text="Anti-recoil started - auto-updating active...")
        else:
            self.running = False
            self.status_label.config(text="🔴 STOPPED", fg='#ff0000')
            self.start_button.config(text="START ANTI-RECOIL", bg='#00ff00')
            self.debug_label.config(text="Anti-recoil stopped")
    ...
    def move_mouse_anti_recoil(self, dx, dy):
        """Anti-recoil mouse movement method with anti-detection"""
        try:
    ...
    def start_auto_update_loop(self):
        """Start the anti-recoil auto-updating loop"""
        def anti_recoil_loop():
            ...
            while True:
                try:
                        ...
                        if mouse_pressed:
                            ...
                            if caps_lock_on:
                                ...
                                if current_time - last_mouse_time >= delay:
                                    # Update debug info
                                    self.success_count += 1
                                    debug_text = f"🎯 ANTI-RECOIL - Auto recoil: {aimdown} pixels. Success: {self.success_count}"
                                    self.debug_label.config(text=debug_text)
                                    
                                    # Move mouse using anti-recoil method
                                    success = self.move_mouse_anti_recoil(0, aimdown)
                                    
                                    if success:
                                        self.debug_label.config(text=f"✅ ANTI-RECOIL movement successful! Success: {self.success_count}")
                                    else:
                                        self.error_count += 1
                                        self.debug_label.config(text=f"❌ ANTI-RECOIL movement failed! Errors: {self.error_count}")
                    ...
                    self.error_count += 1
                    self.debug_label.config(text=f"Anti-recoil error: {str(e)} - Auto-fixing...")
                    time.sleep(0.1)
        
        # Start the loop in a separate thread
        self.anti_recoil_thread = threading.Thread(target=anti_recoil_loop, daemon=True)
        self.anti_recoil_thread.start()
    
    def on_closing(self):
        """Handle closing"""
        if self.running:
            if messagebox.askokcancel("Quit", "Anti-recoil app is running. Quit?"):
                self.root.destroy()
        else:
            self.root.destroy()
