...
class ImprovedNoRecoilApp:
    ...
    def initialize_app(self):
        ...
        def update_progress(step_index):
            if step_index < len(steps):
                ...
                # Update progress bar
                width = int(progress * 400)  # 400 is the width of loading frame
    ...
    def recoil_loop(self):
        ...
        while self.running:
            if self.recoil_enabled.get():
                if not self.require_ads.get() or ms.is_pressed("right"):
                    ...
                    ms.move(horizontal, vertical, absolute=False)
                    time.sleep(delay)
            time.sleep(0.01)
